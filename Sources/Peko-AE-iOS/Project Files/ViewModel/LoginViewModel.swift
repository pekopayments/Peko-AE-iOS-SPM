...
public struct LoginViewModel {
    ...
    @MainActor public func userLogin(username:String, password:String, response: @escaping (ResponseModel<LoginDataModel>?, _ error: Error?) -> Void) {
        let parameter = [
            "username":username,
            "password":password
        ]
    ...
    @MainActor public func logout(response: @escaping (CommonResponseModel?, _ error: Error?) -> Void) {
